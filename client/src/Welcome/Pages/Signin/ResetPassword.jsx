...
import { resetAndUpdatePasswordFun, resetPasswordFun, resetUsernameSuccess } from '../../../Redux/action';
...
function ResetPassword() {
    ...
    const { resetPasswordLoadingFlag, resetPasswordSuccessData } = useSelector((state) => state);
    ...
    const inputUserDetailStep2 = (event) => {
        ...
        if (inputBoxSecurityQuestion.securityAnswer1 === resetPasswordSuccessData.user.securityAnswer1 && inputBoxSecurityQuestion.securityAnswer2 === resetPasswordSuccessData.user.securityAnswer2) {
            handleComplete()
        } else {
    ...
    const inputUserDetailStep3 = (event) => {
        event.preventDefault();
        inputBoxNewPassword.id = resetPasswordSuccessData.user._id;
    ...
    const [valuesConfirmPasswordViewPart, setvaluesConfirmPasswordViewPart] = React.useState(false);
    ...
    if (resetPasswordSuccessData !== null && resetPasswordSuccessData !== undefined && flagOneTimeValid && resetPasswordSuccessData.success) {
        handleComplete()
        setFlagOneTimeValid(false)
    } else if (resetPasswordSuccessData !== null && resetPasswordSuccessData !== undefined && flagOneTimeValid && !resetPasswordSuccessData.success) {
                                ...
                                {activeStep === 0 ?
                                    <form className='welcome_resetusername_formOutsideBox' onSubmit={inputUserDetailStep1}>
                                        ...
                                        <Stack direction="row" spacing={1} style={{ margin: "auto" }}>
                                            <Button type='submit'>{resetPasswordLoadingFlag ? <img src={freeLoadGif} alt="" style={{ width: "50px" }} /> : "Next Step"}</Button>
                                            <Button onClick={handleReset}>Close</Button>
                                        </Stack>
                                    </form>
                                    : activeStep === 1 ?
                                        ...
                                        <form className='welcome_resetusername_formOutsideBox' onSubmit={inputUserDetailStep3}>
                                            ...
                                            <FormControl sx={{ m: 1, width: "100%" }} variant="standard">
                                                ...
                                                />
                                                {inputBoxNewPassword.confirmPassword === "" || inputBoxNewPassword.password === inputBoxNewPassword.confirmPassword ? "" : <p style={{ color: "red", textAlign: 'start' }}>Enter the same password</p>}

                                            </FormControl>
                                            <Stack direction="row" spacing={1} style={{ margin: "auto" }}>
                                                <Button type='submit' disabled={buttonDisable}>Update Password</Button>
                                                <Button onClick={handleReset}>Close</Button>
                                            </Stack>
                                        </form>
                                }
